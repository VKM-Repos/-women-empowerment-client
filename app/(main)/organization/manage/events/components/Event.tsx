...
export default function Event({image, name, organization, date, venue, popMenu}: EventCardProps) {
  ...
  return (
    <div className='flex justify-between items-center border border-gray-500 p-5 rounded-md w-[700px]'>
        ...
        <div className='flex items-center gap-5'>
            <div>
                <Image src={image} alt='organization logo' className='w-16' />
            </div>
            <div className='flex flex-col'>
                <h3 className='text-xl font-semibold font-sora'>{name}</h3>
                <p className='text-lg text-gray-300 font-quickSand font-bold'>{organization}</p>
                <p className='text-xs text-info font-medium'>{date} <span>({venue})</span> </p>
            </div>
        </div>
        <div>
