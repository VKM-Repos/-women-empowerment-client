...
export default function Event({
  ...
  const eventDraftMenu = [
    ...
    {
      title: "Delete",
      link: `/projects/delete/${project?.id}`,
      icon: (
        <svg
          width="16"
          height="16"
  ...
  return (
    <div className="cursor-pointer hover:bg-slate-200 flex justify-between items-center border border-gray-500 p-2 rounded-md w-[650px]">
      <Link href={`/projects/${project?.id}`}>
        <div className="flex items-start gap-5">
          <div
            className={`w-[${imageWidth ? imageWidth : 150}px] h-[${
          ...
          >
            <Image
              src={project?.image}
              alt="Project image"
              className={`w-[${imageWidth ? imageWidth : 150}px] max-w-[${
                imageWidth ? imageWidth : 150
              }px] aspect-square object-cover`}
              width={imageWidth ? imageWidth : 150}
              height={imageWidth ? imageWidth : 150}
            />
          </div>
          <div className="flex flex-col items-start">
            <h3 className="text-md font-semibold font-sora text-gray-300">
              {project?.title}
            </h3>
            <p
              className={`text-sm  font-quickSand text-black-100 ${
                imageWidth > 100 ? "w-[350px]" : ""
              }`}
            >
              {project?.description.substring(0, 55)} .....
            </p>
            <button className="text-xs bg-white-100 shadow-md mt-3 px-2 py-1 rounded-md text-primary font-light">
              {project?.status}{" "}
            </button>
          </div>
        </div>
        ...
        {includeMenu && (
          <div className="relative">
            <Menu
              ...
              menuItems={projectStatus == "Drafts" ? eventDraftMenu : eventMenu}
            />
          </div>
        )}
        {includeMenu && (
