...
const projectMenu = [
    {
        title: "Edit",
        link: '/organization/manage/projects/edit',
  ...
  return (
    <div>
        <div className='w-full flex justify-between my-10'>
          ...
          <div>
            <button className='px-5 py-2 bg-btnWarning rounded-md font-light font-sora text-white-100'>Add Project</button>
          </div>
        </div>
        <div className='flex flex-col gap-5'>
          { selectedEventType.name == 'All Projects' &&        
            [1,2,3,4]?.map(event =>(
              <Project
                image={projectImaage} 
                name='A 360 Project'
                organization='To ignite transforming change in Nigeria’s Education sector, fostering an ecosystem where education, excellence is recognized and celebrated'
                status='Ongoing'
                popMenu={projectMenu}
                />
            ) )
          }
          {
            ...
            [1]?.map(event =>(
              <Project
                image={projectImaage} 
                name='Particulars Project'
                organization='To ignite transforming change in Nigeria’s Education sector, fostering an ecosystem where education, excellence is recognized and celebrated'
                status='Ongoing'
                popMenu={projectDraftMenu}
                />
            ) )
          }
        </div>
